'''
Extract features for UCF101 24 classes annotation
...
def load_frames(img_list, video_root_path, num_frames=15):
    ...
    for line in lines:
        ...
        for i in range(0, len(img_index_list)):
            img_name = os.path.join(video_path, 'frame' + str('%06d'%(img_index_list[i])) + '.jpg')
...
def main():
    ...
    all_frames, all_frame_names, all_labels = load_frames(
                                                img_list = "./UCF101_spatial_localization/annotation/new_ucf101_24_train.txt",
    ...
    for i in range(toal_num_video):
        ...
        features = features.view(20, 2048, 49)
